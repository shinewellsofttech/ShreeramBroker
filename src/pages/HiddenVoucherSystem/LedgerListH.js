...
import { Card, CardBody, Input, Button, Table, Spinner, Badge, Modal, ModalHeader, ModalBody } from 'reactstrap';
...
const LedgerListH = () => {
    ...
    const fetchDataList = useCallback(async () => {
        try {
            ...
            await Fn_FillListData(
                ...
                setState,
                'ledgerData',
                `${API_WEB_URLS?.MASTER || ''}/0/token/OtherLedgers/Id/0`
            );
        } catch (error) {
            console.error('Error fetching hidden ledger list:', error);
        } finally {
    ...
    const filteredData = useMemo(() => {
        ...
        if (statusFilter !== 'all') {
            data = data.filter(item => statusFilter === 'active' ? item.IsActive : !item.IsActive);
        }
                    ...
                    <Table hover className="align-middle table-nowrap mb-0 table-sm text-nowrap">
                        <thead className="table-light" style={{ position: 'sticky', top: 0, zIndex: 10 }}>
                            <tr>
                                ...
                                <th className="text-center" style={{ backgroundColor: '#f8f9fa' }}>Status</th>
                            ...
                            {state.loading ? (
                                <tr>
                                    <td colSpan="3" className="text-center py-5">
                                        <Spinner color="primary" size="sm" className="me-2" /> Loading Ledgers...
                                    </td>
                                </tr>
                            ) : filteredData.length === 0 ? (
                                <tr>
                                    <td colSpan="3" className="text-center py-4 text-muted">No ledgers found.</td>
                                </tr>
                            ) : (
                                filteredData.map((item, idx) => (
                                    <tr key={idx}>
                                        <td className="fw-bold" style={{ whiteSpace: 'normal', minWidth: '200px' }}>{item.Name}</td>
                                        ...
                                        <td className="text-center">
                                            <Button color="soft-info" size="sm" onClick={() => handleEdit(item.Id)} title="Edit Ledger">
